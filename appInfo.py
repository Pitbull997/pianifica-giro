...
def scarica_db_da_github():
    """
    Scarica il database aggiornato direttamente dal branch principale del tuo repo GitHub.
    Sostituisci l'URL raw con il link effettivo del tuo file CSV su GitHub.
    """
...
if tot_clienti > 0:
    tot_qta = int(st.session_state.giro_corrente['Q.ta'].sum()) if 'Q.ta' in st.session_state.giro_corrente.columns else 0
    tot_comuni = int(st.session_state.giro_corrente['COMUNE'].nunique()) if 'COMUNE' in st.session_state.giro_corrente.columns else 0
    ...
    if st.session_state.vista_pulita:
        ...
        st.markdown("<p style='color: #94A3B8; font-size: 13px;'>💡 Trascina le schede per riordinare le fermate del giro:</p>", unsafe_allow_html=True)
