...
# ==========================================
# SCHERMATA 0: WELCOME / HOME PAGE CON LOGIN
# ==========================================
if not st.session_state.autenticato:
    ...
    if os.path.exists(img_path):
        ...
        # Overlay interattivo in Streamlit sopra lo sfondo
        st.markdown('<div style="position: fixed; bottom: 3%; left: 50%; transform: translateX(-50%); width: 90%; max-width: 380px; z-index: 100001; background: rgba(18, 18, 18, 0.85); padding: 20px; border-radius: 16px; border: 2px solid #60A5FA;">', unsafe_allow_html=True)
        st.markdown("<h3 style='text-align: center; color: white; margin-top: 0;'>🚐 ACCESSO VANGO</h3>", unsafe_allow_html=True)
        
        username = st.text_input("Username", key="login_user")
        password = st.text_input("Password", type="password", key="login_pass")
        
        if st.button("ENTRA", use_container_width=True, type="primary"):
            # CONFIGURA QUI I DATI DI ACCESSO ACCETTABILI
            USER_VALIDO = "admin"
            PASS_VALIDA = "vango2026"
            
            if username == USER_VALIDO and password == PASS_VALIDA:
        ...
        st.markdown("### 🚐 ACCESSO VANGO")
        username = st.text_input("Username", key="login_user_fallback")
        password = st.text_input("Password", type="password", key="login_pass_fallback")
        if st.button("ENTRA", use_container_width=True, type="primary"):
            if username == "admin" and password == "vango2026":
                st.session_state.autenticato = True
                st.session_state.pagina_attiva = "giro"
                st.rerun()
            else:
