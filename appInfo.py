...
if "pagina_corrente" not in st.session_state:
    st.session_state.pagina_corrente = "splash"
...
# ==========================================
# 3. SCHERMATA DI LOGIN / SPLASH SCREEN
# ==========================================
if not st.session_state.logged_in:
    st.markdown("<h1 style='text-align: center; color: #0D9488;'>VanGo</h1>", unsafe_allow_html=True)
    st.markdown("<h4 style='text-align: center; color: #94A3B8; letter-spacing: 2px;'>ROUTE PLANNER</h4>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; font-size: 15px; color: #E2E8F0; margin-bottom: 25px;'>Your Smart Delivery Solution</p>", unsafe_allow_html=True)
    
    col_s1, col_s2, col_s3 = st.columns(3)
    with col_s1:
        st.markdown("<div style='text-align: center;'>📍<br><b>Ottimizza</b><br><span style='font-size:12px; color:#94A3B8;'>i percorsi</span></div>", unsafe_allow_html=True)
    with col_s2:
        st.markdown("<div style='text-align: center;'>⏱️<br><b>Risparmia</b><br><span style='font-size:12px; color:#94A3B8;'>tempo</span></div>", unsafe_allow_html=True)
    with col_s3:
        st.markdown("<div style='text-align: center;'>⛽<br><b>Riduci</b><br><span style='font-size:12px; color:#94A3B8;'>i costi</span></div>", unsafe_allow_html=True)
        
    st.markdown("<hr style='margin: 30px 0; border-color: #333333;'>", unsafe_allow_html=True)
    
    # Sezione di Accesso integrata nella schermata
    st.markdown("<p style='text-align: center; font-weight: bold; color: #38BDF8;'>🔑 ACCESSO AL TERMINALE</p>", unsafe_allow_html=True)
    
    col_l1, col_l2, col_l3 = st.columns([1, 2, 1])
    with col_l2:
        with st.form("form_login"):
            input_user = st.text_input("Username (es. autista / admin)")
        ...
        # Pulsanti rapidi per testare subito l'accesso senza digitare
        st.markdown("<p style='text-align: center; font-size: 12px; color: #94A3B8; margin-top: 15px;'>Accesso rapido di prova:</p>", unsafe_allow_html=True)
        col_b1, col_b2 = st.columns(2)
        with col_b1:
            if st.button("🚀 Entra come Autista", use_container_width=True):
                st.session_state.logged_in = True
                st.session_state.username = "autista"
                st.session_state.ruolo = "autista"
                st.session_state.giro_corrente = scarica_db_da_github()
                st.session_state.pagina_corrente = "home"
                st.rerun()
        with col_b2:
            if st.button("🛠️ Entra come Admin", use_container_width=True):
                st.session_state.logged_in = True
...
with st.sidebar:
    ...
    if st.button("🚀 Schermata Iniziale", use_container_width=True):
        st.session_state.pagina_corrente = "splash"
        st.rerun()
